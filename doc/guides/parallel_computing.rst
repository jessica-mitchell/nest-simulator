...
assigned to the recorder. Spike files have the file extension :term:`gdf` and
analog recordings from the ``multimeter`` have :term:`dat` as file extension.
...
:term:`Connection` object are always handled by the virtual process of the
`target node`.
